...
results <- omicsflow(
    ...
    # cache      = "my_gene_coords.rds" # Optional: Custom Ensembl hg38 cache for CNV
)
```

::: {.callout-note title="CNV Preprocessing & Cache"}
If you include CNV data (`cnv = "cnv.rds"`), the pipeline requires an Ensembl gene coordinates cache (`cache`) to map segments to genes. Because querying Ensembl dynamically can take several minutes and is prone to network timeouts, OmicsFlow uses a bundled default cache automatically. You can download an up-to-date hg38 cache for production use by running `generate_cnv_cache("my_gene_coords.rds")` and passing it explicitly.
:::
