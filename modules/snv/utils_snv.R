# ==============================================================================
# utils_snv.R — Shared Utilities for the SNV Module
...
snv_step <- function(step_num, title) {
  width <- 62
  message(sprintf("\n\u250c%s\u2510", strrep("\u2500", width)))
...
#' Load all packages required for SNV preprocessing
load_snv_packages <- function() {
...
#' Read raw SNV mutation data
load_snv_data <- function(snv_file) {
...
add_snv_qc <- function(qc_list, category, key, value) {
  qc_list[[category]][[key]] <- value
  return(qc_list)
}

#' Export the complete QC tracking list to a JSON file
export_snv_qc <- function(qc_list, outdir) {
  ...
  snv_msg(sprintf("Saved QC metrics: %s", out_path))
}
