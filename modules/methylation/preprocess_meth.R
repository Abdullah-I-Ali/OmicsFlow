...
if (!is.null(metadata)) {
  ...
  sample_type <- tryCatch(meth_extract_sample_type(colnames(met_beta)), error=function(e) rep(NA, ncol(met_beta)))
  if (all(is.na(sample_type))) {
    meth_msg(sprintf("No TCGA sample types detected. Keeping all %d samples.", ncol(met_beta)))
    qc <- meth_update_qc(qc, "samples", "primary_tumor", ncol(met_beta))
    
    na_counts   <- colSums(is.na(met_beta))
    met_beta    <- met_beta[, order(na_counts), drop = FALSE]
    met_beta    <- met_beta[, !duplicated(colnames(met_beta)), drop = FALSE]
    
    meth_msg(sprintf("After deduplication: %d unique samples", ncol(met_beta)))
    qc <- meth_update_qc(qc, "samples", "after_deduplication", ncol(met_beta))
  } else {
...
if (!is.null(metadata)) {
  ...
  st <- tryCatch(meth_extract_sample_type(colnames(met_temp)), error=function(e) rep(NA, ncol(met_temp)))
  if (all(is.na(st))) {
    na_counts_temp   <- colSums(is.na(met_temp))
    keep_idx <- tapply(seq_len(ncol(met_temp)), colnames(met_temp),
                       function(idx) idx[which.min(na_counts_temp[idx])])
    keep_idx <- sort(unlist(keep_idx))
    met_temp <- met_temp[, keep_idx, drop = FALSE]
    
    orig_barcodes <- colnames(met_temp)
    names(orig_barcodes) <- orig_barcodes
  } else {
